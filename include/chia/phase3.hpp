					...
					DiskTable<S>* R_table = nullptr)
{
	...
	std::thread R_sort_read(
		[&mutex, &signal_1, num_threads, num_threads_read, L_table, R_sort, R_table, &R_read, &R_is_end]() {
			if(R_table) {
				R_table->read(&R_read, num_threads_read);
			} else {
				R_sort->read(&R_read, std::max(num_threads / (L_table ? 1 : 2), 1), num_threads_read);
			}
			R_read.close();
			{
	...
	if(L_table) {
		L_table->read(&L_read_1, num_threads_read);
		L_read_1.close();
	} else {
		L_sort->read(&L_read, std::max(num_threads / (R_table ? 1 : 2), 1), num_threads_read);
	}
	L_read.close();
	{
		std::lock_guard<std::mutex> lock(mutex);
						...
						FILE* plot_file, uint64_t L_final_begin, uint64_t* R_final_begin)
{
	...
	R_sort->read(&R_read, num_threads, std::max(2 * num_threads / g_read_thread_divider, 2));
	R_read.close();
