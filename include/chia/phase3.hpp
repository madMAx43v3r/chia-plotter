				...
				const std::string tmp_dir_2)
{
	...
	const std::string prefix = tmp_dir + plot_name + ".";
	...
	auto R_sort_lp = std::make_shared<DiskSortLP>(
			2 * k - 1, log_num_buckets, prefix + "p3s1.t2");
	...
	for(int L_index = 2; L_index < 6; ++L_index)
	{
		...
		R_sort_lp = std::make_shared<DiskSortLP>(
				2 * k - 1, log_num_buckets, prefix + "p3s1." + R_t);
	...
	R_sort_lp = std::make_shared<DiskSortLP>(2 * k - 1, log_num_buckets, prefix + "p3s1.t7");
